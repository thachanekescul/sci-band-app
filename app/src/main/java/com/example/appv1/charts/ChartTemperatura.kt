// File: app/src/main/java/com/example/appv1/admin/charts/ChartTemperatura.kt
...
class ChartTemperatura(
    ...
    fun loadChart() {
        val layout = LayoutInflater.from(context)
            .inflate(R.layout.item_grafica_tmp, container, false) as LinearLayout
        container.addView(layout)

        val txtNombrePaciente = layout.findViewById<TextView>(R.id.txtNombrePaciente)
        ...
        fun updateChart(days: Int, ids: List<String>) {
            val collection = if (days == 1) "bio-datosprm" else "bio-datosprd"
            val collectionRef = db.collection(
                "organizacion/$organizacionId/cuidadores/$cuidadorId/pacientes/$pacienteId/$collection"
            )
            ...
            Tasks.whenAllSuccess<DocumentSnapshot>(tasks)
                .addOnSuccessListener { results ->
                    ...
                    for ((index, id) in ids.withIndex()) {
                        ...
                        val valor = doc.getLong("t")?.toFloat()
                        if (valor != null) {
        ...
        btn1Dia.setOnClickListener { updateChart(1, listOf("01", "12", "24", "48")) }
        btn15Dias.setOnClickListener { updateChart(15, listOf("01", "04", "07", "15")) }
